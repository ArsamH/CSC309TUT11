...
function update_stats() {
  ...
  /* TODO: Complete these stats calculations. 
    ...
    */
  let total_seconds = timers.reduce(
    (accumulator, timer) => accumulator + timer.total_seconds(),
    0
  );
  let num_expired = timers.filter((timer) => timer.expired).length;
  let num_active = timers.filter((timer) => !timer.expired).length;
...
function create_timer(event, form) {
  ...
  let timer = new Timer(
    ...
    (m, s) => {
      ...
      minutes.forEach((elem, _i) => {
        elem.innerHTML = m;
      });
      seconds.forEach((elem, _i) => {
        elem.innerHTML = String(s).padStart(2, "0");
      });
    },
    remove
  );
